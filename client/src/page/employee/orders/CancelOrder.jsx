...
const CancelOrder = () => {
  ...
  const [selectStatus, setSelectStatus] = useState("Requested_Canceled")
    ...
    <div className="h-screen bg-white flex flex-col">
      ...
      <div className="flex gap-4 items-center justify-center mb-2">
        <button className="p-1 hover:text-blue-500 cursor-pointer"
          onClick={() => setSelectStatus("Requested_Canceled")}
        >
          Requested_Canceled
        </button>
        <button className="p-1 hover:text-blue-500 cursor-pointer"
          onClick={() => setSelectStatus("Canceled_Refunded")}
        >
          Canceled_Refunded
        </button>
                ...
                orders.map((order) => (
                  <tr key={order.order_id} className="hover:bg-gray-50">
                    ...
                    {(order.order_status === "Requested_Canceled" || order.order_status === "Canceled_Refunded") && (
                      <td className="p-4 text-red-500">{order.order_status}</td>
                    )}

                    <td className="p-3">{order.total_amount}฿</td>

                    {order.order_status !== "Canceled_Refunded" && (
                      <td className="p-3">
